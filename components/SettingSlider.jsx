...
const SettingSlider = ({ label, minValue = 0, maxValue = 100, valChange, storeKey, desc }) => {
    const [value, setValue] = useState(1)

    useEffect(() => {
        const getVal = async() => {
            const val = await AsyncStorage.getItem(storeKey) || `${minValue}`;
            setValue(Number(val));
        }
        getVal();
    }, [storeKey])
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={[styles.text, {paddingLeft: scale(10), fontSize: scale(12), color: '#A1968F'}]}>
                {desc}
            </Text>
            <Slider
                style={styles.slider}
